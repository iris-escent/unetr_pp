...
class UnetrPPEncoder(nn.Module):
    def __init__(self, input_size=[16 * 40 * 40, 8 * 20 * 20, 4 * 10 * 10, 2 * 5 * 5], dims=[32, 64, 128, 256],
                 proj_size=[64, 64, 64, 32], depths=[3, 3, 3, 3], num_heads=4, spatial_dims=3, in_channels=1, dropout=0.0,
        ...
        for i in range(4):
            ...
            for j in range(depths[i]):
                ...
                stage_blocks.append(TransformerBlock(input_size=input_size[i], hidden_size=dims[i], proj_size=proj_size[i],
                                                    num_heads=num_heads, dropout_rate=transformer_dropout_rate,
                                                    pos_embed=True))
...
class UnetrUpBlock(nn.Module):
    def __init__(
            ...
            edge_channels: int = 0,  # 新增：接收边缘图的通道数
    ) -> None:
        """
        Args:
            ...
            edge_channels: channels of the incoming edge map for fusion.
        """
        super().__init__()
        # 上采样层
        ...
        if self.edge_channels > 0:
            self.edge_fusion_conv = get_conv_layer(
                spatial_dims,
                out_channels + edge_channels,  # 拼接后的总通道数
                out_channels,  # 降维回原来的 out_channels
                kernel_size=1,  # 使用 1x1x1 卷积平滑融合
                stride=1,
                conv_only=True,
            )
    ...
    def forward(self, inp, skip, edge_feat=None):
        # 新增：接收 edge_feat
        out = self.transp_conv(inp)
        out = out + skip
        # 新增：如果传入了边缘图且配置了边缘通道，则进行拼接和融合
        if self.edge_channels > 0 and edge_feat is not None:
            out = torch.cat([out, edge_feat], dim=1)  # 在通道维度拼接
            out = self.edge_fusion_conv(out)  # 卷积特征融合
