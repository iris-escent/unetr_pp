...
class MobileMambaModuleSeq(nn.Module):
    #通道维 C 按比例切成三段：global全局 / local局部 / identity剩余
    ...
    def forward(self, x):  # x: [B, N, C]
        ...
        x1 = self.global_ffn(x1)  #分别处理
        x2 = self.local_ffn(x2)
        out = torch.cat([x1, x2, x3], dim=-1) #拼接
...
class TransformerBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        # 3D → 序列化 tokens [B,N,C]
        x_reshaped = x.reshape(B, C, H * W * D).permute(0, 2, 1)
        ...
        attn_skip = x_mamba.reshape(B, H, W, D, C).permute(0, 4, 1, 2, 3)
        ## [B, C, H, W, D]
